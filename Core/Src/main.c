...
int main(void) {
	...
	while (1) {
		if (packetReady) {
			...
			if (rxPacketLen > 0 && rxPacketLen % 2 == 0) {
			    HAL_GPIO_TogglePin(LD2_GPIO_Port, LD2_Pin); // confirms we got here

			    for (uint16_t i = 0; i < rxPacketLen; i += 2) {
			        uint8_t ledId = rxPacket[i];
			        uint8_t state = rxPacket[i + 1];
			        GPIO_PinState pinState = (state == 0x01) ? GPIO_PIN_SET : GPIO_PIN_RESET;
